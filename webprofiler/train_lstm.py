...
print(" - use_peepholes: {0:b}".format(use_peepholes))
with tf.variable_scope(train_model, initializer=tf.variance_scaling_initializer()):
    if use_peepholes == True:
        lstm_cells = [tf.contrib.rnn.LSTMCell(num_units=n_neurons, activation=activation, use_peepholes=use_peepholes)
				for layer in range(n_layers)]
    else: # False
        lstm_cells = [tf.contrib.rnn.BasicLSTMCell(num_units=n_neurons, activation=activation)
				for layer in range(n_layers)]
